...
m_buffer(NULL)
{
	m_state  = new wxUint32[8U];
	m_total  = new wxUint32[2U];
	m_buffer = new wxUint32[32U];
...
/* Copy the value from v into the memory location pointed to by *cp,
   ...
   * (uint32_t *) cp = v  */
static inline void set_uint32(unsigned char* cp, wxUint32 v)
{
	wxASSERT(cp != NULL);
...
//# define UNALIGNED_P(p) (((unsigned int) p) % alignof (wxUint32) != 0)
...
static const wxUint32 roundConstants[64] = {
...
void CSHA256::processBlock(const unsigned char* buffer, unsigned int len)
{
	...
	const wxUint32* words = (wxUint32*)buffer;
	unsigned int nwords = len / sizeof(wxUint32);
	const wxUint32* endp = words + nwords;
	wxUint32 x[16];
	wxUint32 a = m_state[0];
	wxUint32 b = m_state[1];
	wxUint32 c = m_state[2];
	wxUint32 d = m_state[3];
	wxUint32 e = m_state[4];
	wxUint32 f = m_state[5];
	wxUint32 g = m_state[6];
	wxUint32 h = m_state[7];
