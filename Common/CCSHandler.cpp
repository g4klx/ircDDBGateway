...
void CCCSHandler::process(CAMBEData& data)
{
	...
	if (m_state == CS_CONNECTED) {
		...
		WriteJSONLinking(m_local.ToStdString(), "network", "ccs", m_yourCall.ToStdString());
	} else {
	...
	switch (type) {
		case CT_TERMINATE:
			if (m_state == CS_ACTIVE) {
				...
				WriteJSONUnlinked(m_local.ToStdString(), "user");
				m_stateChange = true;
...
void CCCSHandler::startLink(const wxString& dtmf, const wxString& user, const wxString& type)
{
	...
	if (!callsign.IsEmpty()) {
		...
		WriteJSONLinking(user.ToStdString(), "user", "ccs", callsign.ToStdString());
		...
		WriteJSONLinking(user.ToStdString(), "user", "ccs", dtmf.ToStdString());
		m_yourCall = dtmf;
...
void CCCSHandler::clockInt(unsigned int ms)
{
	...
	if (m_pollInactivityTimer.isRunning() && m_pollInactivityTimer.hasExpired()) {
		...
		WriteJSONRelinking(m_local.ToStdString(), "ccs", m_yourCall.ToStdString());
	...
	if (m_inactivityTimer.isRunning() && m_inactivityTimer.hasExpired()) {
		...
		WriteJSONUnlinked(m_local.ToStdString(), "timer");
